...
class SubscriberContractEditDetailsHandler(BaseHandler):
...
class SubscriberContractEditMixin:
    ...
    @memorizing
    def sco(self):
        return self.db_session.query(SubscriberContractOrder).filter_by(
            id=self.data['contract_edit_id'],
            client_id=self.client_id,
            deleted=None
        ).first()
...
class SubscriberContractCancelHandler(BaseHandler):
    ...
    def _post(self, *args, **kwargs):
        ...
        if confirm is None:
            return {
                "error": 1,
                "reply": self._t("confirm_wrong_sms_code"),
            }
